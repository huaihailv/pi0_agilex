...
def process_multiple_tasks(raw_dirs, repo_id: str, robot_type: str = "aloha", mode: str = "video"):
    ...
    for task_index, task_dir_str in enumerate(raw_dirs):
        ...
        for ep_file in task_dir.glob("episode_*.hdf5"):
            ...
            dataset.save_episode(task=selected_instruction,task_index=task_index)
    dataset.consolidate()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--raw_dirs", type=str, nargs="+", required=True,
                    help="List of task directories to be processed.")
    ...
    process_multiple_tasks(args.raw_dirs, args.repo_id, args.robot_type, args.mode)
    
# uv run examples/aloha_real/convert_aloha_multidir_to_lerobot.py --raw_dirs /mnt/hpfs/baaiei/robot_data/agilex/build_blocks/task_building_blocks_left, /mnt/hpfs/baaiei/robot_data/agilex/build_blocks/task_building_blocks_right --repo_id HuaihaiLyu/build_blocks --robot_type aloha --mode video
